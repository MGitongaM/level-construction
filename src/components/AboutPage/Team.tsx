...
export default function Team() {
    const teamMembers=[
        {
            ...
            academics:["MSc in Mechatronics Engineering(JKUAT)",
        ...
        },
        {
            ...
            academics:["MSc in project Management (JKUAT)",
        ...
        },
        {
            ...
            academics:["BSc in Mechanical Engineering (TUM)",
                "Registered Engineer",
            ],
        },
         {
  ...
  return (
    <>
    <section className="container mx-auto px-4 pb-10 -mt-30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 my-10">
            {
                ...
                teamMembers.map((member)=>(<Card key={member.id} className=" border bg-teal-50/20  hover:scale-105 transition duration-700">
                    <CardHeader className="flex flex-row gap-8">
                        ...
                        {member.image}
                        <div className="c">
                        <CardTitle className="font-extrabold">{member.firstName} {member.lastName}</CardTitle>
                        <CardDescription className="text-xl">{member.postition}</CardDescription>
                        </div>
                    </CardHeader>
                    <CardContent className="">
                        <Accordion type="single" collapsible >
                                <AccordionItem  value="Areas of Expertise">
                                    ...
                                    {member.expertise.map((strength)=>(
                                        <AccordionContent key={strength} className="ml-10 flex items-center gap-2"><FileCheck size={10} className="text-teal-300"/> {strength}</AccordionContent>
                                    ))}
                                    </div>
                                </AccordionItem>
                        </Accordion>
                        <Accordion type="single" collapsible >
                                <AccordionItem  value="Academic & Professional Qualifications">
                                    <AccordionTrigger className="font-semibold hover:no-underline">Academic & Professional Qualifications:</AccordionTrigger>
                                    {member.academics.map((strength)=>(
                                        <AccordionContent key={strength} className="ml-10 flex items-center gap-2"><BookCheckIcon size={10} className="text-teal-300"/> {strength}</AccordionContent>
                                    ))}
                                </AccordionItem>
                        </Accordion>
                    </CardContent>
